...
// Utilities
function copy_to_clipboard(codeEl, headerEl) {
  var txt = document.getElementById("copy");
  txt.value = codeEl.innerText;
  txt.select();
  txt.setSelectionRange(0, 99999);
  document.execCommand("copy");
  headerEl.innerText = 'Copied!';
}

function copy_code_to_clipboard() {
  copy_to_clipboard(
    document.querySelector('#upload-instructions code'),
    document.querySelector('#browser-header')
  );
}